# 星辰空间矩阵(Star Matrix Terminal)
> 极客级纯前端加密货币量化终端 | 零后端依赖 | 直连交易所API





星辰空间矩阵是一款面向专业交易者的轻量级加密货币量化看盘终端，基于纯前端技术栈构建，无需部署任何后端服务，数据直接对接主流交易所官方API，极致轻量化且隐私安全。
...
npm run dev
```
引擎启动成功后，在浏览器中访问终端输出的本地地址（默认通常为 `http://localhost:5173`）即可开启星辰雷达。
